...
2. **각 항목 = 3-step 루프**:
   ...
   - (b) `implementation_guide.acceptance`에 명시된 테스트를 실행. 테스트 명령 탐지 절차:
     1. **escape hatch**: `.deep-review/config.yaml`의 `test_command` 필드 존재 시 사용
     2. `hooks/scripts/test/test-*.sh` 존재 시 전체 실행
     3. `package.json` scripts.test → `npm test` (lockfile로 `pnpm test` 구분)
   ...
   - (c) 결과를 `log_path`에 append. 포맷 엄격 준수:
     - 시작: `===== ITEM-{id} START {ISO8601-timestamp} =====`
     - 끝: `===== ITEM-{id} END exit={code} =====`
     - 합성: `bash -c 'cmd 2>&1 | tee -a "$log_path"'` 패턴 (stdout+stderr 합쳐서 append)
     - 백그라운드 금지. 순차 append로 라인 순서 보존.
