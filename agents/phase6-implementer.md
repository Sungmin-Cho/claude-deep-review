...
2. **각 항목 = 3-step 루프**:
   ...
   - (c) 결과를 Main이 제공한 `Constraints.log_path`(절대 경로) 파일에 append. **`log_path`는 prompt data이므로 shell 변수처럼 `$log_path`로 참조하지 말고, Main이 프롬프트에 전달한 절대 경로 문자열을 그대로 literal로 명령에 삽입**:
     ...
     - **실행 패턴** — Main이 전달한 `log_path`를 해당 명령에 **literal로 substitute한 뒤 실행**. 예를 들어 Main이 `log_path: /Users/alice/repo/.deep-review/tmp/phase6-critical.log` 를 전달했다면, 실제로 실행할 Bash 명령은:
       ```bash
       bash -c 'cmd 2>&1 | tee -a /Users/alice/repo/.deep-review/tmp/phase6-critical.log'
       ```
       (`/Users/alice/repo/...`는 예시일 뿐 — **당신에게 Main이 실제로 전달한 절대경로**로 대체해야 한다. `/abs/.../` 같은 placeholder 문자열을 그대로 복사하면 안 된다.)
