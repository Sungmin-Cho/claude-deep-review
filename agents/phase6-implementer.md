...
2. **각 항목 = 3-step 루프**:
   ...
   - (c) 결과를 Main이 제공한 `Constraints.log_path`(절대 경로) 파일에 append. **`log_path`는 prompt data이므로 shell 변수처럼 `$log_path`로 참조하지 말고, Main이 프롬프트에 전달한 절대 경로 문자열을 그대로 literal로 명령에 삽입**:
     ...
     - **실행 패턴** — Main이 전달한 `log_path`를 명령에 **literal로 substitute하되, shell quote로 감싼다**. `log_path`에 공백·glob·`$`·backtick 등 shell metachar가 포함될 수 있으므로 **항상 single-quote로 래핑**해야 `tee`가 단일 인자로 받는다.
