...
export async function runClaudeReviewer(options = {}) {
  ...
  const model = requiredString(options.model ?? 'opus', 'model');
...
async function main() {
  ...
  if (options.help) {
    process.stdout.write('Usage: run-claude-reviewer.mjs --project-root DIR --plugin-root DIR --prompt-file FILE --output FILE [--model MODEL] [--agent NAME] [--timeout-seconds N]\n');
    return;
  }
