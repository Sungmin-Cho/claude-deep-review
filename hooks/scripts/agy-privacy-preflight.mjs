...
async function main() {
  ...
  if (options.help) {
    process.stdout.write('Usage: agy-privacy-preflight.mjs --repo DIR --plugin-root DIR --approval auto|approve|decline [--config FILE]\n');
    return;
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  main().catch((error) => {
    process.stderr.write(`agy-privacy-preflight.mjs: ${error.message}\n`);
    process.exitCode = 2;
  });
}
