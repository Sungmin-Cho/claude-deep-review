...
M3 Phase 2 envelope adoption (handoff §3 procedure). `.deep-review/recurring-findings.json` is now emitted as an M3 cross-plugin envelope (cf. `claude-deep-suite/docs/envelope-migration.md`). Reader paths that consume deep-work session-receipt are envelope-aware with a strict 3-way identity guard. The 5th of 6 plugins to migrate; deep-evolve (PR #11 `9b867b1`) is the immediate predecessor and the most relevant pattern reference.
