# review-execution — `/deep-review` 리뷰 모드 절차 (라우터에서 on-demand Read)

<!-- commands/deep-review.md 의 리뷰 분기에서 Read 되어 그대로 수행된다. 동작 SSOT. -->
...
### 5.5 Recurring Findings Export (precheck)

**리포트 저장 + `last_review` 갱신(위 5.3/5.3a) 이후** 평가한다. `.deep-review/reports/` 의 리포트 수(**방금 저장한 리포트 포함**)가 **2 미만이면 건너뛴다**. 2 이상이면 전체 export 절차를 참조에서 Read 하고 그대로 수행한다:

`Read({ file_path: "${CLAUDE_PLUGIN_ROOT}/skills/deep-review-workflow/references/recurring-findings-export.md" })`

### 6. 엔트로피 스캔 (--entropy)

`--entropy` 플래그가 있을 때만 전체 스캔 절차를 참조에서 Read 하고 그대로 수행한다:

`Read({ file_path: "${CLAUDE_PLUGIN_ROOT}/skills/deep-review-workflow/references/entropy-scan.md" })`
