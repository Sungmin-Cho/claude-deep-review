...
| N=2 | 0/2 | 🟢 APPROVE |
| N=1 | 1/1 지적 (🔴 critical/security) | 🔴 REQUEST_CHANGES — "단일 리뷰어" 표기 유지 (severity 자체가 blocking, 단독이라도 non-blocking 로 흘리지 않음) |
| N=1 | 1/1 지적 (🟡 only) | 🟡 단독 — 리포트에 "단일 리뷰어" 주의 표시 후 CONCERN |
| N=1 | 0/1 | 🟢 APPROVE (단, "단일 리뷰어"로 표기) |

리포트의 Summary에 `Review Mode: {N-way, executed=[reviewer list]}`로 실제 구성을 명시.

> **(SSOT 정합)** 위 N=1 행은 `review-execution.md` §5.1 "N_actual == 1 (1-way) 전용 분기" 및 `report-format.md` verdict 규칙과 **동일 매핑**이다(1/1 🔴 → REQUEST_CHANGES + 단일 리뷰어 표기 / 1/1 🟡 → CONCERN + 표기 / 0/1 → 🟢 APPROVE + 표기). critical/security 는 단독 리뷰어라도 blocking(severity 자체가 verdict 를 지배) — 세 문서는 같은 결론을 담으며 값 drift 를 두지 않는다.
