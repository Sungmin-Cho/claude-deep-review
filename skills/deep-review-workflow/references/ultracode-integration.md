...
> Related: [`codex-integration.md`](./codex-integration.md) (Codex 교차검증), [`review-criteria.md`](./review-criteria.md) (5개 리뷰 관점).
...
`--ultracode` 는 단일 Opus 서브에이전트를 **5개 병렬 리뷰어 샤드**로 교체한다 (`review-criteria.md` 의 5관점):
...
각 샤드는 기존 `agents/code-reviewer.md` 에이전트를 **prompt 로 한 관점에 집중**시켜 spawn(에이전트 frontmatter 변경 불필요). 각 샤드 prompt 에는 단일 Opus 리뷰어와 **동일한 컨텍스트**(diff, rules.yaml, fitness.json, health_report, contract)를 포함한다. 차원 수는 5로 고정(가변 샤드는 백로그).
...
**(A) Workflow 경로 (Claude Code + Workflow 가용):** `Workflow` 도구 1회 호출 — Phase "Review"(5 차원 `agentType: code-reviewer`, 구조화 출력) → Phase "Verify"(finding 별 적대적 refute, pipeline). refute 된 finding 은 **무음 삭제하지 않고** `confidence = low` 로 강등·보존한다(§4 VOICE-6).
...
- **identity 키**: `{file}:{line_bucket}` 만 사용. **severity·category 는 키에서 제외.**
  ...
  - category 제외: 5렌즈(샤드)와 7-카테고리 taxonomy 는 1:1 이 아님 — 같은 model·같은 diff 이므로 **같은 줄 ≈ 같은 이슈**. category 로 쪼개면 이중 계산. 7-taxonomy 는 downstream recurring-findings/loop signature 전용으로 분리.
...
> ⚠️ **핵심 invariant**: 5 샤드는 same-family(Anthropic). 각각을 cross-model 표로 세면 합의 통계가 왜곡되므로 반드시 한 보이스로 collapse 한 뒤 codex/agy 와 합산한다.
