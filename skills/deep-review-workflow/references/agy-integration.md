...
**Resolution chain**: `AGY_FINGERPRINT_MODE` env var > config field > built-in default `hybrid`. The orchestrator (`review-execution.md` Stage 3) resolves before invoking the bridge; the bridge receives the resolved value via `--mode`.
