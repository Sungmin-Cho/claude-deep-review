...
**Main의 절차**: 정식 구현은 `commands/deep-review.md`의 `--respond` Step 2.5를 단일 소스로 한다. 실행 가능 검증은 `hooks/scripts/test/test-phase6-protocol-e2e.sh` (E1~E6). 본 문서는 스킬 로드 시점의 요약이며 상충 시 **`commands/deep-review.md`가 우선**한다.
