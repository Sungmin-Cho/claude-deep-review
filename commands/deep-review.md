...
3. **Pre-dispatch snapshot** — 워크스페이스 baseline + allowlist 수집 (dirty-tree 지원 + 파싱 실패 복구 + trust boundary):
   ```bash
   # baseline: 그룹 시작 직전 상태 (dirty tree 포함)
   ...
   PRE_STATUS=$(git status --porcelain=v1 -uall)
   # allowlist: 이 그룹의 accepted items의 target_location 경로들
   ALLOWED=$(printf '%s\n' "${TARGET_LOCATIONS_OF_GROUP[@]}" | cut -d: -f1 | sort -u)
   ```
...
5. **결과 검증 (dispatch 성공 시) — fail-closed + delta 기반 + allowlist**:
   - 반환 메시지에 `## Group Result` 블록이 **없거나 파싱 불가능**하면 dispatch 실패로 재분류 → Step 7 "Dirty recovery"로 분기 (main fallback 직행 금지).
   - 블록 존재 시 post-dispatch 상태 산출:
     ```bash
     ...
     # delta = post - pre (이 그룹이 실제로 바꾼 것만; rename은 --name-status로 보정 필요 시)
     DELTA=$(comm -13 <(echo "$PRE_ALL") <(echo "$POST_ALL"))
     ```
   - 서브에이전트 claim과 delta 비교 (authoritative = git delta, NOT claim):
     ```bash
     CHANGED_FILES_CLAIM=("<서브에이전트 files_changed 원본>")
     CLAIM=$(printf '%s\n' "${CHANGED_FILES_CLAIM[@]}" | sort -u)
     if [[ "$CLAIM" != "$DELTA" ]]; then
       execution_status="error"
       error_reason="files_changed mismatch: claim != git delta"
       # detail을 response.md notes에 기록, Step 7로 분기
     fi
     ```
   - **Allowlist 강제** — delta가 ALLOWED 외 경로 포함 시 차단:
     ```bash
     OUTSIDE=$(comm -23 <(echo "$DELTA") <(echo "$ALLOWED"))
     if [[ -n "$OUTSIDE" ]]; then
       execution_status="error"
       error_reason="files outside allowlist: $OUTSIDE"
     fi
     ```
   - `log_path` 존재 가드:
     ```bash
     if [[ ! -f "$log_path" ]]; then
       ...
       # 파싱 불가 항목에 대해 execution_status=error (log 없이는 판정 불가)
     fi
     ```
   - 실패 항목은 `Read(log_path)` — `ITEM-{id}` 구분자로 해당 구간 tail 확인.
   - **검증 통과 시**: `CHANGED_FILES=(DELTA의 항목들)` 확정.
6. **그룹 커밋 — pathspec-limited**: 전원 PASS(`items_passed == items_total`) **AND** Step 5 검증 통과 시에만:
   - **빈 배열 가드**: `[[ ${#CHANGED_FILES[@]} -eq 0 ]]`면 "전원 PASS인데 변경 0건" — 이상 상태로 `execution_status=error` 처리.
   - **Untracked 신규 파일 처리** — `git commit --only`는 untracked 파일을 포함 못함. 새 파일은 `git add` 먼저:
     ```bash
     NEW_FILES=$(comm -13 <(echo "$PRE_UNTRACKED") <(echo "$POST_UNTRACKED"))
     if [[ -n "$NEW_FILES" ]]; then
       # 신규 파일만 명시적 add (그 외 기존 tracked는 --only가 처리)
       echo "$NEW_FILES" | xargs -r git add --
     fi
     ```
   - **Pre-staged 인덱스 안내** — `git commit --only`는 지정 경로만 커밋하고 pre-staged 인덱스는 건드리지 않음:
     ```bash
     if ! git diff --cached --quiet -- . ":(exclude)${CHANGED_FILES[@]}" 2>/dev/null; then
     ...
     ```
   - **커밋** — `-m`은 반드시 `--` **앞에** (pathspec parsing 방지):
     ```bash
     git commit --only -m "fix(review-response): resolve {severity} items from {report_basename}" -- "${CHANGED_FILES[@]}"
     ```
   - **금지**: `git add -A`, `git commit` (pathspec 없이), `git commit --only -- <paths> -m <msg>` (`-m`이 pathspec으로 오인).
   - 부분 실패/halted/error 그룹은 커밋 건너뜀. response.md에 "워킹 트리에 passed 항목 수정 남음" 경고 기록.
7. **Dirty recovery (파싱 실패 / error status / dispatch 실패 공통)**:
   - Post snapshot과 baseline 비교: `POST_STATUS=$(git status --porcelain=v1 -uall)`.
   - `PRE_STATUS != POST_STATUS` (서브에이전트가 dirty 상태 남김) → AskUserQuestion:
     > "서브에이전트가 파일을 수정한 상태로 실패/중단했습니다. 어떻게 할까요?
     > (1) 파일 수정을 유지하고 main이 직접 Phase 6 계속 수행 (partial edits 위에 쌓음 — 권장 안 함)
     > (2) baseline으로 restore 후 main이 직접 Phase 6 수행 (`git restore --source=HEAD -- <delta_files>`)
     > (3) 여기서 중단, 사용자가 수동 판단"
   - (2) 선택 시 `git restore` + `git clean` (untracked 신규 파일 삭제)으로 baseline 복원 → main fallback 진입.
