...
**실행 가능한 e2e 테스트**: 아래 각 Step의 핵심 shell 로직은 `hooks/scripts/test/test-phase6-protocol-e2e.sh`의 8개 테스트(E1~E8)에서 실증 검증됨. pseudocode가 문서로 drift하지 않도록 **CI에서 e2e도 함께 실행**.
...
3. **Pre-dispatch snapshot** (spec §5.4.1) — allowlist + content-aware baseline:
   ...
   # unstaged rename은 보통 delete + untracked로 분해되므로 실제로는 staged rename에
   # 주로 적용. Binary 파일은 --name-status도 경로 단위로 나열되며 DELTA는 아래
   # `git hash-object`(binary 대응 가능)로 계산되므로 별도 분기 없음 (E8 검증).
   PRE_MODIFIED=$(git -c core.quotepath=false diff --name-status -M | awk -F'\t' '
     $1 ~ /^[RC]/ { print $3; next }
     $1 != "" { print $2 }
   ' | sort -u)
...
5. **결과 검증 (dispatch 성공 시) — fail-closed + content-aware delta + allowlist** (spec §5.4.2~§5.4.7):
   ...
   - **Path-set violation** — ALLOWED 외부 경로가 수정/생성됐는지 별도 체크 (trust boundary):
     ...
     POST_MODIFIED=$(git -c core.quotepath=false diff --name-status -M | awk -F'\t' '
       $1 ~ /^[RC]/ { print $3; next }
       $1 != "" { print $2 }
     ' | sort -u)
