...
- `Bash({ command: 'node "{codex_companion_path}" review --base {review_base}', run_in_background: true })`
- `Bash({ command: 'node "{codex_companion_path}" adversarial-review --base {review_base} "{focus_text}"', run_in_background: true })`

{codex_companion_path}는 환경 감지에서 얻은 경로.
Skill tool이 아닌 Bash tool로 직접 호출하는 이유: Codex 플러그인의 review/adversarial-review 커맨드에 disable-model-invocation: true가 설정되어 있어 Skill tool로는 호출 불가.
