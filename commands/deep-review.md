...
`--respond` 전체 절차(자동 복원/tmp 회전 → 리포트 로딩 → receiving-review Phase 1~5 → Phase 6 dispatch → Response 저장 → Re-review)는 참조에 있다. **이 파일을 Read 하고 그대로 수행한다:**
...
init 모드 전체 절차(Step 1~9)는 참조에 있다. **이 파일을 Read 하고 그대로 수행한다:**
