...
**실행 가능한 e2e 테스트**: 아래 각 Step의 핵심 shell 로직은 `hooks/scripts/test/test-phase6-protocol-e2e.sh`의 10개 테스트(E1~E10)에서 실증 검증됨. pseudocode가 문서로 drift하지 않도록 **CI에서 e2e도 함께 실행**.
...
7. **Dirty recovery — per-file content baseline** (spec §5.4.9, E5 검증):
   ...
   - AskUserQuestion:
     ...
     > (3) 중단, 사용자 수동 판단"
   - (2) 선택 시 — **`git restore --source=HEAD` 사용 금지** (pre-existing WIP 파괴). 대신 Step 3에서 저장한 per-file baseline으로 **worktree + index 양쪽**을 복원 (W4 교정: subagent 가 `git add`/`git mv` 로 index 를 건드렸을 수 있음 — worktree 만 복원하면 staged 잔존):
     ```bash
     # ALLOWED 경로만 baseline content로 복원 (사용자의 non-ALLOWED dirty는 건드리지 않음)
     while IFS= read -r f; do
       [[ -z "$f" ]] && continue
       # (W4) 먼저 index 상태를 PRE 로 되돌림 — subagent 의 git add/mv 영향 제거.
       # PRE 가 tracked (파일 존재) 였으면 `git restore --staged` 로 HEAD 버전으로 un-stage.
       # PRE 가 untracked ("absent") 였으면 `git rm --cached` 로 index 에서 제거.
       if [[ "${PRE_HASH[$f]}" == "absent" ]]; then
     ...
     ```
     - 주의: `git restore --staged` 는 사용자가 Phase 6 진입 전 동일 path 에 일부러 stage 한 hunk 도 함께 un-stage 한다. Subagent 개입과 분리 불가능 — Phase 6 계약 상 ALLOWED 내 path 는 Phase 6 전용이라 pre-existing staged 가 "의도된 사용자 hunk" 일 가능성은 낮지만, recovery 직후 response.md 에 복원 로그를 남겨 사용자가 확인 가능하도록 한다.
