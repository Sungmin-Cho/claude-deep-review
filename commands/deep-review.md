...
4. **결과 검증 (dispatch 성공 시) — fail-closed**:
   - 반환 메시지에 `## Group Result` 블록이 **없거나 파싱 불가능**하면 Step 3의 dispatch 실패로 재분류 → main fallback 분기(§6.2).
   - 블록 존재 시: **git을 authoritative source**로 삼아 실제 변경 파일 집합 산출 (untracked 포함, 서브에이전트 claim에 의존하지 않음):
     ```bash
     git_modified=$(git diff --name-only)
     git_untracked=$(git ls-files --others --exclude-standard)
     GIT_CHANGED=$(printf '%s\n%s\n' "$git_modified" "$git_untracked" | sort -u | sed '/^$/d')
     ```
   - 서브에이전트 주장 `files_changed`와 비교. **불일치 시 해당 그룹을 error로 마킹, commit 건너뜀, PR posting 억제**, response.md에 `notes`로 경고 기록 (경고 기록만으로 진행하지 않는다):
     ```bash
     CLAIM=$(printf '%s\n' "${FILES_CHANGED_FROM_SUBAGENT[@]}" | sort -u)
     if [[ "$CLAIM" != "$GIT_CHANGED" ]]; then
       execution_status="error"
       error_reason="files_changed mismatch: subagent claim != git state"
       # diff detail은 response.md notes에 기록
     fi
     ```
   - `log_path` 파일이 존재하지 않거나 Read 실패 시도 동일하게 error 처리 (`log_unavailable: true`).
   - 실패 항목은 `Read(log_path)` 또는 tail — `ITEM-{id}` 구분자로 해당 구간 확인.
5. **그룹 커밋 — pathspec-limited**: 전원 PASS(`items_passed == items_total`) **AND** 검증 통과 시에만:
   - 서브에이전트 `files_changed`가 `GIT_CHANGED`와 일치함을 Step 4에서 확인했으므로 `CHANGED_FILES` = 그 교집합.
   - **`git commit --only` 사용** — pre-staged 인덱스를 절대 포함하지 않는다:
     ```bash
     # 커밋 직전 pre-staged 인덱스 유무를 명시적으로 체크 (정보 제공용)
     if ! git diff --cached --quiet; then
       echo "⚠ Pre-staged files detected outside Phase 6 scope:"
       git diff --cached --name-only | grep -vxF -f <(printf '%s\n' "${CHANGED_FILES[@]}") || true
       echo "→ Using 'git commit --only -- <paths>' to exclude them from this commit."
     fi
     git commit --only -- "${CHANGED_FILES[@]}" -m "fix(review-response): resolve {severity} items from {report_basename}"
     ```
   - **`git add -A` 금지 + `git commit` (no --only) 금지** — `git commit --only` 가 인덱스 무관하게 지정 경로만 커밋한다는 점을 활용 (Stage 1 민감 파일 방지 + pre-staged 인덱스 흡수 방지).
   - 부분 실패 또는 halted 그룹은 커밋 건너뜀. response.md에 "워킹 트리에 passed 항목 수정 남음" 경고 기록.
